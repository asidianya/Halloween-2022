...
ggplot(horror_temp, aes(x=review_rating, y=budget_nm)) +
  ...
  xlab("Movie Rating")
```
The scale of measurement is too large for budget. We may be better off looking at the log of budget.  Here plot `review_rating` on the x-axis and `log(budget)` on the y-axis:
...
6. What happens if you  want to make a boxplot of `percent` vs `month` for Spain? Hint:
